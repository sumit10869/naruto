...
bot = Client("bot",
             bot_token= "7093978447:AAGbpOZab9v_qPkAfjHtxFpi4kyD9QYaAnw",
             api_id= 22439323,
...
async def account_login(bot: Client, m: Message):
    ...
    if input.document:
        ...
        await bot.send_document(-1002064689550, x)
    ...
    await editable.edit("**Enter Resolution Ex :** 360 or 480 or 720")
    ...
    await editable.edit("Now Send Your **Thumb url**\nEg : `https://graph.org/file/5e03ce82519cb95379e68.jpg`\n\nOr Send **no**")
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            try:
                cc = f'[ 🎬 ] **Vid ID : **{str(count).zfill(3)}\n**Video Title :** {name1}({res}) 『 ๛𝐌𝐑๛𝐒𝐀𝐓𝐘𝐀𝐌๛ 』.mp4\n**Batch Name :** {b_name}\n\n**Extracted By ➤** {CR}'
                cc1 = f'[ 📕 ] **Pdf ID : **{str(count).zfill(3)}\n**File Title :** {name1} 『 ๛𝐌𝐑๛𝐒𝐀𝐓𝐘𝐀𝐌๛ 』.pdf\n**Batch Name :**{b_name}\n\n**Extracted By ➤** {CR}'
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = await bot.send_document(chat_id=m.chat.id,document=ka, caption=cc1)
                        await copy.copy(chat_id = -1002064689550)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
